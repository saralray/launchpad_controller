...
- **`launchpad/midi.py`** — `MidiSurface`: port discovery, hot-plug detection, LED output. `open()` blocks/retries until both Launchpad in/out ports appear; never lets exceptions escape. Port selection goes through `device.pick_launchpad_port`, which returns the **first** Launchpad port the OS exposes (same port keychecker and the manage GUI use, for in and out alike). **Startup layout is forced on (re)connect**: `MidiSurface.startup_layout` holds a select-layout SysEx value that `open()` sends every time the device appears (so hot-plug reconnect re-applies it). `app.main()` sets it to `app.STARTUP_LAYOUT` (currently `device.CUSTOM_3` = the device's "User"-labelled tab). Set `startup_layout = None` to restore the old "leave the surface in whatever layout it powers up in" behavior, or `device.PROGRAMMER_LAYOUT` for the full-surface documented numbering. Because Custom modes only route/light the **8x8 grid** (top CC row + scene column go dark), config.json's `room_key` CC selectors won't work in a custom layout — use `PROGRAMMER_LAYOUT` if you need the top row. Whatever layout is forced, config.json and the calibration wizard must match the note/CC numbers that layout actually emits. `device.select_layout_sysex` (command `00`, values `CUSTOM_1/2/3`, `PROGRAMMER_LAYOUT`) and the older `device.layout_sysex` / `midi.set_programmer_mode` (programmer/live toggle, command `0E`) are both available.
...
- **Room model** (`config.json`): a list of `rooms`, each with a `room_key` (`control_change` number — top-row buttons select the active room) and `actions` (`note_on` numbers — grid buttons). Only one room is active at a time (`Controller.active_room`); grid presses act on `active_room.actions` only. On startup `Controller._initial_room()` picks the first room whose name contains "preset" (case-insensitive), falling back to `rooms[0]` — so the daemon boots on the Presets tab.
